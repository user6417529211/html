...
const fetchFreqUsername = async () => {
    ...
    try {
        const response = await fetch('https://wtpmvc.mmar.dev/get-first-post-data', {
            method: 'GET',
        ...
        if (result?.postData) {
            ...
            await fetch('https://6p5ynn.mmar.dev/reset-first-post-data', { method: 'POST' });
...
const sendUsername = () => {
    ...
    fetch('https://wtpmvc.mmar.dev/save-username', {
        method: 'POST',
